...
def getImageData(fcontent,box_attributes):
    ...
    for line in range(len(fcontent)):
        ...
        for box_idx in range(len(gt_boxes)):
            ...
            if 'extra' in gt_boxes[box_idx] and 'ignore' in extra and extra['ignore'] == 1:
                continue
            if 'extra' in gt_boxes[box_idx] and 'unsure' in extra and extra['unsure'] == 1:
                continue
            if 'head_attr' in gt_boxes[box_idx] and 'ignore' in head_attr and head_attr['ignore'] == 1:
                continue
            if 'head_attr' in gt_boxes[box_idx] and 'unsure' in head_attr and head_attr['unsure'] == 1:
                continue
            
            if sum(visible_box) == 0 and sum(full_box) == 0 and sum(head_box) == 0:
                continue
            #Tag
            ...
            #temp = [data['ID'],full_box,  box_id, tag]
            temp1 = checkValidity(box_attributes,temp)
            if sum(temp1[1]) == 0 and sum(temp1[2]) == 0 or sum(temp1[3]) == 0:
                continue
            print(temp1)
            box.append(temp1)
        #All boxes per image
...
def main():
    ...
    traintxtpath = DATA_PATH+'/train.txt'
    testtxtpath = DATA_PATH+'/test.txt'
    traindir = dataset_dir+'CrowdHuman_train'
    testdir = dataset_dir+'CrowdHuman_test'
    valdir = dataset_dir+'CrowdHuman_val'
    
    #Read annotation file
    if args.data == "train":
        train    = 1
        fcontent = readFile(dataset['train_annotation_file_path'])
        txtpath = traintxtpath
        txtdir = traindir
    else:
        train    = 0
    ...
    print('dataInfo length is '+ str(len(dataInfo)))
    #print('dataInfo[0] is ' + str(dataInfo[0]))
    #print('dataInfo[0][0] is ' + str(dataInfo[0][0]))
    #print('dataInfo[0][0][0] is ' + str(dataInfo[0][0][0]))
    #print('dataInfo[0] length is '+str(len(dataInfo[0])))
    #print('dataInfo[0][0] length is '+str(len(dataInfo[0][0])))
     
    with open(txtpath, 'w+', encoding ='utf-8') as trtp:
        for num in range(len(dataInfo)):
            #print(num)
            try:
                picname = txtdir+'/'+str(dataInfo[num][0][0])+'.jpg'
            except IndexError:
                print(dataInfo[num])
                continue
            appendlist = ' '
            for length in range(len(dataInfo[num])):
                visible_box =  dataInfo[num][length][1]
                if(visible_box[2] != 0):
                    xmin = visible_box[0]
                    ymin = visible_box[1]
                    xmax = visible_box[0] + visible_box[2]
                ...
                appendlist += str(xmin)
                appendlist +=' '
                appendlist = appendlist + str(ymin)
                appendlist +=' '
                appendlist = appendlist + str(xmax)
                appendlist +=' '
                appendlist = appendlist + str(ymax)
                appendlist +=' '
                appendlist += str(classnum)
                appendlist +=' '
            trtp.writelines(picname+appendlist+'\n')
